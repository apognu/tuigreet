  ...
  match input {
    ...
    KeyEvent { code: KeyCode::Enter, .. } => match greeter.mode {
      ...
      Mode::Command => {
        ...
        greeter.sessions.selected = greeter.sessions.options.iter().position(|Session { command, .. }| Some(command) == cmd.as_ref()).unwrap_or(0);
      ...
      Mode::Sessions => {
        let session = match greeter.sessions.options.get(greeter.sessions.selected) {
